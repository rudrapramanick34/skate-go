      ...
      return `
        <article class="product-card ${isOutOfStock ? 'out-of-stock-card' : ''}">
          ...
          <h3 class="product-card-title">
            <a href="product.html?slug=${p.slug}" style="color:#FFF; text-decoration:none;">${p.title}</a>
          </h3>

          <p style="font-size: 0.8rem; color: #BDBDBD; margin-bottom: 0.75rem; line-height:1.4;">
            ${p.description ? p.description.slice(0, 90) + '...' : ''}
          </p>

          <div class="product-card-price">₹${(Number(p.price) || 0).toLocaleString('en-IN')}</div>

          ${isOutOfStock ? `
            <button class="btn btn-outline btn-sm btn-full" disabled style="opacity: 0.5; cursor: not-allowed; border-color: #555; color: #888;">
              OUT OF STOCK
            </button>
          ` : `
